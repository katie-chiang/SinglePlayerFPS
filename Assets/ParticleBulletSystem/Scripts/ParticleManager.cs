...
namespace ParticleBulletSystem
{
    ...
    public class ParticleManager : MonoBehaviour
    {
        ...
        public void changeVelocity(int number, Vector3 startPosition, Vector3 endPosition, float emitSpeed)
        {
            Vector3 newVelocity = (endPosition - startPosition).normalized;
            newVelocity = newVelocity * emitSpeed;
            ParticleSystem par = particle[number];
            int length = par.GetParticles(particles[number]);
            // Debug.Log("particle length: " + length);
            // for (int i = 0; i < length; i++)
            // {
            //     particles[number][i].velocity = newVelocity;
            // }

            //only change the direction of the newest particle
            particles[number][length-1].velocity = newVelocity;
